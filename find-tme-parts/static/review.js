...
function renderRows() {
  ...
  for (const row of state.rows) {
    ...
    status.className = `status status-${(row["TME Match Status"] || "unknown").replaceAll(" ", "_")}`;
...
function renderDetails(row, product) {
  ...
  document.querySelector("#part-description").textContent = product.description || "No TME description supplied.";
  document.querySelector("#final-count").value = row["Final Item Count"];
  const links = document.querySelector("#part-links");
  links.replaceChildren();
  for (const [label, url] of [["Open at TME", product.product_url], ["Datasheet", product.datasheet_url]]) {
    if (!url) continue;
...
document.querySelector("#count-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await requestJson(`/api/rows/${state.selected}/count`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ count }) });
...
document.querySelector("#substitution-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const result = await requestJson(`/api/rows/${state.selected}/substitution`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ symbol }) });
...
reloadRows().catch((error) => { message.textContent = error.message; });
