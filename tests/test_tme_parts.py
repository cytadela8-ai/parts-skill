...
class ReviewAppTests(unittest.TestCase):
    ...
    def test_approval_sets_the_final_match_status(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            final_path = Path(directory) / "bom-final.csv"
            final_path.write_text(
                "Reference,Value,Footprint,Qty,TME Match Status,TME Match Notes\n"
                "R1,10k,Resistor_SMD:R_0402_1005Metric,10,needs_review,Check package\n",
                encoding="utf-8",
            )
            app = review_parts.create_app(final_path, lambda: "token")
            response = app.test_client().post("/api/rows/0/approve")
            with final_path.open(encoding="utf-8", newline="") as file:
                row = next(csv.DictReader(file))
